...
/***************************************************************************/	
	...
	function themify_page() {
		...
		<div id="content">
            ...
            <div id="setting" class="maintab">
                ...
                <ul class="subtabnav">
                    ...
                    foreach($config['panel']['settings']['tab'] as $tab){
                        if($x){
                            echo '<li class="selected"><a href="#setting-'.themify_scrub_func($tab['title']).'">'.$tab['title'].'</a></li>';	
                            $x = 0;
                        } else {
                            echo '<li><a href="#setting-'.themify_scrub_func($tab['title']).'">'.$tab['title'].'</a></li>';	
                        }
                    } 
                    ?>
                </ul>
                 ...
                 <?php foreach($config['panel']['settings']['tab'] as $tab){ ?>					
                    ...
                    <div id="setting-<?php echo themify_scrub_func($tab['title']); ?>" class="subtab">
                        <?php 
            ...
            <div id="styling" class="maintab">
                ...
                <ul class="subtabnav">
                    <?php 
                    $x = 1;
					if(isset($config['panel']['styling']['tab']['title'])){
						echo '<li class="selected"><a href="#styling-'.themify_scrub_func($config['panel']['styling']['tab']['title']).'">'.$config['panel']['styling']['tab']['title'].'</a></li>';	
					} else {
						foreach($config['panel']['styling']['tab'] as $tab){
							if($x){
								echo '<li class="selected"><a href="#styling-'.themify_scrub_func($tab['title']).'">'.$tab['title'].'</a></li>';	
								$x = 0;
							} else {
								echo '<li><a href="#styling-'.themify_scrub_func($tab['title']).'">'.$tab['title'].'</a></li>';	
							}
						}
					}
					?>
				...
				?>
					<!-- subtab: styling-<?php echo themify_scrub_func($config['panel']['styling']['tab']['_a']['title']); ?> -->
						<div id="styling-<?php echo themify_scrub_func($config['panel']['styling']['tab']['title']); ?>" class="subtab">
							<?php 
							if(is_array($config['panel']['styling']['tab']['element'])){
								if(isset($config['panel']['styling']['tab']['element']['title']) && isset($config['panel']['styling']['tab']['element']['selector'])){
									echo themify_container(themify_scrub_func($tab['title']), $config['panel']['styling']['tab']['element']); 
								} else {
									foreach($config['panel']['styling']['tab']['element'] as $element){
										echo themify_container(themify_scrub_func($config['panel']['styling']['tab']['title']), $element); 
									}
								}
							}
							?>
				...
				<?php 
					...
					foreach($config['panel']['styling']['tab'] as $tab){ ?>					
						<!-- subtab: styling-<?php echo themify_scrub_func($tab['_a']['title']); ?> -->
						<div id="styling-<?php echo themify_scrub_func($tab['title']); ?>" class="subtab">
							<?php 
							if(is_array($tab['element'])){
								if(isset($tab['element']['title']) && isset($tab['element']['selector'])){
									echo themify_container(themify_scrub_func($tab['title']), $tab['element']); 
								} else {
									foreach($tab['element'] as $element){
										echo themify_container(themify_scrub_func($tab['title']), $element); 
									}
								}
							}
							?>
        ...
        <!-- footer -->
        <div id="bottomtab">
           <p id="logo"><span>Themify</span> <a href="http://themify.me/logs/framework-changelogs/" target="_blank" class="themify_changelogs">v<?php echo $version; ?></a></p>
            <p class="reset">
				...
				foreach($config['styling'] as $nav => $value){
					foreach($value as $element => $val){
						...
						if($temp != '' && $temp != ' '){
							...
							$style_selector = themify_get_styling_selector('id', $element, true);
							if ( $style_selector != '' ) {
...
function themify_meta_field_color($args, $call_before_after = true, $echo = true){
	...
	<input type="button" class="button clearColor" value="' . __('Clear', 'themify') . '">',
		$meta_box['name'], $meta_box['name'], $meta_value? $meta_value : $meta_box['meta']['default'], $class);
	...
	if($meta_box['before']) $html = $meta_box['before'] . $html;
	if($meta_box['after'])  $html .= $meta_box['after'];
...
function themify_post_types($types = array()){
	$defaults = array_merge(array( 'post', 'page' ), themify_specific_post_types());
...
function themify_the_footer_text($area = 'left', $wrap = true, $block = '', $date_fmt = 'Y', $echo = true) {
	...
	// Start markup
	$html = '';
	$html = apply_filters('themify_footer_text'.$block, (themify_get($key) != '')? themify_get($key): $text_block);
 ...
 * @return string
 */
function themify_get_styling_selector( $key, $value, $duplicate = false, $uniq_key = null, $uniq_val = null ) {
	global $ThemifyConfig;
	$config = $ThemifyConfig->get_config();
	$result = '';
	$arr = themify_search_arr( $config['panel']['styling']['tab'], $key, $value, $duplicate, $uniq_key, $uniq_val );
...
function themify_get_google_font_lists() {
	$transient = get_transient( 'themify_google_fonts_transient' );
	...
	if ( false === $transient ) {
		...
		set_transient( 'themify_google_fonts_transient', serialize( $fonts ), WEEK_IN_SECONDS );
	} else {
		$fonts = unserialize( $transient );
	}
	return $fonts;
}
...
function themify_grab_remote_google_fonts() {
	$gfont_api = 'AIzaSyDaaJe1lgwaEoRSuQvu0ZD_b3pzT8Q8Xho';
	...
	if( ! is_wp_error( $response ) ) {
		...
		foreach ( $results->items as $value ) {
			array_push($return, $value->family);
		}
	}
	return $return;
}
...
function themify_enqueue_gfonts() {
	...
	foreach ( $fonts as $font ) {
		...
		array_push( $families, implode('+', $words) . ':' . 400 );
	}
	if(!empty($families)){
...
if(!function_exists('themify_lightbox_vars_init')){
	...
	function themify_lightbox_vars_init(){
		...
		foreach ($file_extensions as $ext) {
			$content_images .= '.post-content a[href$='.$ext.'], .home-highlights-content a[href$='.$ext.'], .slide-content a[href$='.$ext.'], .slide-excerpt a[href$='.$ext.'],';
		...
		$overlay_args = array(
			...
			'deeplinking' 		=> false,
			'contentImagesAreas' => '.post, .type-page, .type-highlights, .type-slider'
		);
...
if(!function_exists('themify_lightbox_link_field')){
	...
	function themify_lightbox_link_field($args = array()) {		
		...
		$lightbox_link = wp_parse_args($args, $defaults);
		return apply_filters('themify_lightbox_link_field', $lightbox_link);
	}
}
